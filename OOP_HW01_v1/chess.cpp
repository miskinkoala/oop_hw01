...
void ChessBoard::move(const string &command)
{
    ...
    {
        cerr << "deneme2" << endl;
        _board[cmd[3]][cmd[2]] = _board[cmd[1]][cmd[0]]; // move to next location
        _board[cmd[1]][cmd[0]] = ChessPiece('.', 'n');   // overwrite location before with '.'
        if (_board[cmd[3]][cmd[2]].getType() == 'k')
        {
            if (_board[cmd[3]][cmd[2]].getColor() == 'b')
            {
                B_king_indexes[0] = cmd[3];
                B_king_indexes[1] = cmd[2];
            }
            else if (_board[cmd[3]][cmd[2]].getColor() == 'w')
            {
                W_king_indexes[0] = cmd[3];
                W_king_indexes[1] = cmd[2];
            }
        }
        delete[] cmd;
        cmd = nullptr;
...
bool ChessBoard::isValidMove(const int *cmd) const
{
    ...
    case 'q':
        ...
        if (search(possible_moves, i_due, j_due) && !(isCheck(piece.getColor()))) // TODO
            flag_valid = true;
        break;
        // TODO check if flag_valid necessery
    case 'k':
        if (i_current - 1 >= 0)
        { // up

            if (_board[i_current - 1][j_current].getType() != '.')
            {
                if (_board[i_current - 1][j_current].getColor() != piece.getColor())
                {
                    possible_moves.push_back({i_current - 1, j_current});
                }
            }
        ...
        if (j_current + 1 <= 7) // right
        {

            if (_board[i_current][j_current + 1].getType() != '.')
            {
                if (_board[i_current][j_current + 1].getColor() != piece.getColor())
                {
                    possible_moves.push_back({i_current, j_current + 1});
                }
            }
            else
                possible_moves.push_back({i_current, j_current + 1});
        }
        if (j_current - 1 >= 0) // left
        {
            if (_board[i_current][j_current - 1].getType() != '.')
            {
                if (_board[i_current][j_current - 1].getColor() != piece.getColor())
        ...
        if (i_current - 1 >= 0 && j_current + 1 <= 7) // cross right up
        {
            if (_board[i_current - 1][j_current + 1].getType() != '.')
            {
                if (_board[i_current - 1][j_current + 1].getColor() != piece.getColor())
                {
                    possible_moves.push_back({i_current - 1, j_current + 1});
                }
            }
        ...
        if (i_current - 1 >= 0 && j_current - 1 >= 0) // cross left up
        {
            if (_board[i_current - 1][j_current - 1].getType() != '.')
            {
                if (_board[i_current - 1][j_current - 1].getColor() != piece.getColor())
        ...
        if (i_current + 1 <= 7 && j_current + 1 <= 7) // cross right down
        {
            if (_board[i_current + 1][j_current + 1].getType() != '.')
            {
                if (_board[i_current + 1][j_current + 1].getColor() != piece.getColor())
        ...
        if (i_current + 1 <= 7 && j_current - 1 >= 0) // cross left down
        {
            if (_board[i_current + 1][j_current - 1].getType() != '.')
            {
                if (_board[i_current + 1][j_current - 1].getColor() != piece.getColor())
